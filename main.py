...
def trigger_transaction(recipient, amount, pool, last_block_hash, clients) -> None:
    ...
    try:
        ...
        if miner1.verify_transaction(sender_obj, new_transaction_1,trans_signature) == True: # at the moment the competition for the fastest proof of work is not active, so it does not make sense to define multiple miners, the one called upon here is thus predefined
        ...
        if len(pool)% 2 == 0: # transactions count per block is 2
            ...
            print('Miner balance: ' + str(miner1.retrieve_balance()))
            transactions_history.set(build_transaction_history(PyCoins_chain))
            last_block_hash = new_block.retrieve_block()[2] # update last_block_hash constant with the new hash generated by the miner
            for i in clients:
                print('Balance of client ' + i.trunc_identity() + ': ' + str(i.retrieve_balance()))
        else:
            print('Transaction recorded. Block not completed yet.')
    except ValueError:
